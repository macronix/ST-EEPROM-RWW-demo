...
static int MxSetDummyLen(MxChip *Mxic, u8 Cmd) {
    ...
    case MX_CMD_4READ_BOTTOM:
        Spi->LenDummy = 6;
        return MXST_SUCCESS; //6
    case MX_CMD_2READ:
        Spi->LenDummy = 4;
        return MXST_SUCCESS; //4
    case MX_CMD_2READ4B:
    case MX_CMD_DREAD:
        Spi->LenDummy = 8;
        return MXST_SUCCESS;
...
static int MxSetAddrLen(MxChip *Mxic, u8 Cmd) {
    ...
    case MX_CMD_4DTRD:
        if (Mxic->ChipSpclFlag & ADDR_3BYTE_ONLY)
            ...
            Spi->LenAddr =
                    (Mxic->SPICmdList[MX_MS_RST_SECU_SUSP] & MX_4B) ?
                            ((Spi->CurAddrMode == SELECT_3B) ? 3 : 4) : 3;
        break;
...
int MxRDBUF(MxChip *Mxic, u32 Addr, u32 ByteCount, u8 *Buf) {
    MxReadTemplate(Mxic, Addr, ByteCount, Buf, MX_CMD_RDBUF);
}
...
uint8_t MxGetStatus(MxChip *Mxic) {
    ...
    case PROT_1_1_1:
        ...
        sCommand.Instruction = MX_CMD_RDSR;
    ...
    if (HAL_OSPI_Command(&OSPIHandle, &sCommand, HAL_OSPI_TIMEOUT_DEFAULT_VALUE)
            != HAL_OK) {
        printf("com failed\r\n");
        return MXST_FAILURE;
    }
    ...
    if (HAL_OSPI_Receive(&OSPIHandle, &reg, HAL_OSPI_TIMEOUT_DEFAULT_VALUE)
            != HAL_OK) {
...
int MxWRSR(MxChip *Mxic, u8 *Buf) {
    ...
    if (Spi->CurMode == MODE_DOPI) {
        ...
        if (Mxic->ChipSpclFlag & SUPPORT_WRSR_CR)
            ByteCount = (Spi->CurMode & MODE_OPI) ? 1 : 2;
        else if (Mxic->ChipSpclFlag & SUPPORT_WRSR_CR2)
            ByteCount = 3;
        else
            ByteCount = 1;
    }
...
int MxRDP(MxChip *Mxic) {
    if (Mxic->SPICmdList[MX_MS_RST_SECU_SUSP] & MX_RDP) {
        ...
        if (Status != MXST_SUCCESS)
            return Status;
        Mx_printf("\r\n  ---111---MxSimpleTest Start!------\r\n");
        return MxSpiFlashWrite(Mxic->Priv, 0, 0, 0, Cmd);
    } else {
        /* if flash does not support,just set CS pin low ,then set CS pin high to exit DP mode*/
#if PLATFORM_ST
        /*set cs pin to gpio function*/
        STM_Set_CSPin_GPIO();

        /* 1) CS# keeps high for minimum 30us*/
        STM_Set_CSPin_High();
        HAL_Delay_Us(30);      //30us

        /* 2) CS# keeps low for minimum 20ns*/
        STM_Set_CSPin_Low();
        HAL_Delay_Us(1);      //1us

        /* 3) As soon as CS# goes high, a maximum delay of 45us is required before releasing from deep power down mode */
        STM_Set_CSPin_High();
        HAL_Delay_Us(45);      //45us

        /*set cs pin to QSPI function*/
        STM_Set_CSPin_OSPI();
#endif
